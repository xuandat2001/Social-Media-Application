Group member contribution information:
Ung Xuan Dat (s3932156) - 5
Pham Quang Huy (s3970891) - 5
Hong Thieu Kiet (s3993986) - 5
Tran Vu Nhat Tin (s3870729) - 5
Nguyen Hoanh Minh (s3915233) - 5

Instructions on how to run the LinkBridge social application program :
Step 1: Unzip the .zip downloaded .zip file containing this README.txt file, Front-end and Back-end folders.

Step 2: Open the folder containing all these mentioned items above on Visual Studio Code.

Step 3: Open the Integrated Terminal for the Back-end folder. 
        Install the following necessary packages and dependencies below:
        npm install nodemon
        npm install ...
        Next, move into the 'server' subfolder using the command "cd .\server\" . Lastly, start the backend server using the command "nodemon .\index.js"

Step 4: Open the Integrated Terminal for the Front-end folder.
        Install all the necessary packages and dependencies by using the command "npm install"
        After finishing installation, in the console, use the command "npm run dev" to start the frontend side of application.
        Finally, hold Ctrl and click on the link included in the console (it should be "http://localhost:5173/").

Step 5: Here are a few test accounts for both users and admin:
        username: jett ; password: 123
        username: Kaido ; password: 123

        For admin: Use the link "http://localhost:5173/admin" with the following account:
        username: admin
        password: admin

Thank you and enjoy using the application!
